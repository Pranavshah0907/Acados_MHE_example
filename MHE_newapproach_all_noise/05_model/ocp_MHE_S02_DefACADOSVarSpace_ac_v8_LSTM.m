...
w_Tr = SX.sym('w_Tr');
w_v = SX.sym('w_v');
w_ht = SX.sym('w_ht',numHiddenUnits);
w_ct = SX.sym('w_ct',numHiddenUnits);

%zeros_for_HU = SX.sym('hu',numHiddenUnits*2+1);

sym_w = vertcat(w_Tw, w_Tr, w_v, w_ht, w_ct); % zeros_for_HU); % 35 values
...
phi = SX.sym('phi', 1);
%v_r = SX.sym('v_r',1 );
p = vertcat(M_EM_acc, M_EM_brk, M_fric_brk, phi); %, v_r); excluded v_r at the moment coz h is not used
