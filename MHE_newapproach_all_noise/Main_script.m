...
pathstr = 'MHE_newapproach_all_noise';
cd([userpath, pathstr]);
...
addpath(genpath('..\MHE_newapproach_all_noise'))
...
nlp_solver_max_iter = 20; %options.sqp_steps; % currently 5
...
qp_solver_max_iter = 100; %options.maxIter;       % currently 30
...
nout = 37; % T_w, T_r, w_Tw, w_Tr
nout_0 = 39; % T_w, T_r, w_Tw, w_Tr, t_w, t_r
...
% Vu = zeros(nout, 2); % new controls = 2
% Vu(3,1) = 1.0; % w_Tw
% Vu(4,2) = 1.0; % w_Tr
% 
% Vu_0 = zeros(nout_0, 2); % new controls = 2
% Vu_0(3,1) = 1.0; % w_Tw
% Vu_0(4,2) = 1.0; % w_Tr

% Old Vu with 35 controls
Vu = zeros(dims.ny, dims.nx);
Vu(3:37,:) = eye(dims.nu);

% Old Vu for 35 controls 
Vu_0 = zeros(nout_0, dims.nu); 
Vu_0(3:37,:) = eye(dims.nu);
...
% 
% input bounds THIS NEES TO BE CHANGED AS THE LIMITS SHOULD BE ON THE NOISE
Jbu = eye(35);
lbu = [0; 0; zeros(33,1)];
...
if open_loop
    ...
    for j=1:N-1
        ...
        estimator.set('p', parameter_input_v2(:,j), j);
    end
    ...
    plot(x_est(1,:))
    hold on
    plot(2:16,y_sim(1,1:N))
    plot(T_wr_orig_v2(1:N,1))
...
if closed_loop
    ...
    plot(x_est(1,:))
    hold on
    plot(y_sim(1,N:N_sim))
    plot(T_wr_orig_v2(N:N_sim,1))
